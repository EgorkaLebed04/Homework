...
function getMessegesHistory(list, maxMessage) {
    allMessages = list.messages.length;
    if (maxMessage > allMessages) {
        maxMessage = allMessages;
    };
    lastMessages = [];
    if (allMessages == 0) {
        ...
        if (maxMessage <= allMessages) {
            for (let i = allMessages-1; i >= (allMessages - maxMessage); i--) {
                lastMessages.unshift(list.messages[i])
            }
        }
    }
    for (let i = 0; i < lastMessages.length; i++) {
        ...
        let rep = message.author.reputation;
        let text = message.text;
        console.log(`${name} (репутация: ${rep}): ${text}`);
    }
}
getMessegesHistory(thread, 3)